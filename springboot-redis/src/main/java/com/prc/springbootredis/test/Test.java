package com.prc.springbootredis.test;
...
public class Test {
    ...
    public static void main(String[] args) {
        ...
        { // doSign
            boolean signed = test.doSign(1000, today);
            if (signed) {
        ...
        { // checkSign
            boolean signed = test.checkSign(1000, today);
            if (signed) {
                System.out.println("您已签到：" + formatDate(today, "yyyy-MM-dd"));
            } else {
        ...
        { // getSignCount
            long count = test.getSignCount(1000, today);
            System.out.println("本月签到次数：" + count);
        }

        { // getContinuousSignCount
            long count = test.getContinuousSignCount(1000, today);
            System.out.println("连续签到次数：" + count);
        }

        { // getFirstSignDate
            LocalDate date = test.getFirstSignDate(1000, today);
            System.out.println("本月首次签到：" + formatDate(date, "yyyy-MM-dd"));
        }

        { // getSignInfo
            System.out.println("当月签到情况：");
            Map<String, Boolean> signInfo = new TreeMap<>(test.getSignInfo(1000, today));
